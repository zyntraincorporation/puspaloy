...
export async function fetchAISettings(): Promise<{ model: string; enabled: boolean; systemPromptAddition: string }> {
  try {
    ...
    if (!snap.exists()) return { model: 'openai/gpt-4o-mini', enabled: true, systemPromptAddition: '' }
    const d = snap.data()
    return {
      model: d.model ?? 'openai/gpt-4o-mini',
      enabled: d.enabled ?? true,
      systemPromptAddition: d.systemPromptAddition ?? '',
    }
  } catch {
    return { model: 'openai/gpt-4o-mini', enabled: true, systemPromptAddition: '' }
  }
}
